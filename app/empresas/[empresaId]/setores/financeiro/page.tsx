...
export default function FinanceiroPage() {
  ...
  function marcarPago(id: string, lista: LancamentoFin[], setter: React.Dispatch<React.SetStateAction<LancamentoFin[]>>) {
    setter((prev) => prev.map((l) => {
      if (l.id !== id) return l;
      audit("Pagamento registrado", "Escritório — A Pagar", `${l.descricao} — ${fmt(l.valor)}`);
  ...
  function marcarRecebido(id: string, lista: LancamentoFin[], setter: React.Dispatch<React.SetStateAction<LancamentoFin[]>>) {
    setter((prev) => prev.map((l) => {
      if (l.id !== id) return l;
      audit("Recebimento registrado", "Escritório — A Receber", `${l.descricao} — ${fmt(l.valor)}`);
      return { ...l, status: "pago" };
    }));
  }
  ...
  return (
    <SetorShell
      ...
      setorResumo="Gestão financeira, mensalidades, cobranças e automações do escritório contábil"
      stats={[
