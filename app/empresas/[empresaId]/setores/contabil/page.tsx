...
const TIPO_CONTA_COLOR: Record<TipoConta, string> = {
  ativo: "#34d399", passivo: "#f87171", pl: "#c4b5fd",
  receita: "#67e8f9", custo: "#fbbf24", despesa: "#374151",
};

const S_LANC: Record<StatusLanc, { bg: string; color: string; label: string }> = {
  rascunho:  { bg: "rgba(251,191,36,0.15)", color: "#fbbf24", label: "Rascunho" },
  conferido: { bg: "rgba(96,165,250,0.15)", color: "#93c5fd", label: "Conferido" },
  fechado:   { bg: "rgba(52,211,153,0.15)", color: "#4ade80", label: "Fechado" },
};
...
const S_CONC: Record<StatusConc, { bg: string; color: string; label: string }> = {
  ...
  conciliado:  { bg: "rgba(52,211,153,0.15)", color: "#4ade80", label: "Conciliado" },
  ignorado:    { bg: "rgba(148,163,184,0.15)", color: "var(--muted)", label: "Ignorado" },
  divergente:  { bg: "rgba(248,113,113,0.15)", color: "#f87171", label: "Divergente" },
};
...
function TH({ children, right, center }: { children: React.ReactNode; right?: boolean; center?: boolean }) {
  return <th style={{ textAlign: right ? "right" : center ? "center" : "left", padding: "0.7rem 0.875rem", color: "#4b6e8e", fontWeight: 700, fontSize: "0.72rem", textTransform: "uppercase", letterSpacing: "0.5px", borderBottom: "1px solid rgba(96,165,250,0.18)", background: "#f8faff" }}>{children}</th>;
}
...
function TD({ children, right, muted, bold, color }: { children: React.ReactNode; right?: boolean; muted?: boolean; bold?: boolean; color?: string }) {
  return <td style={{ padding: "0.75rem 0.875rem", textAlign: right ? "right" : "left", color: color ?? (muted ? "#6b8faa" : "var(--ink)"), fontSize: "0.85rem", borderBottom: "1px solid #f0f6ff", fontWeight: bold ? 700 : 400 }}>{children}</td>;
}
...
export default function ContabilPage() {
  ...
  return (
    ...
    >
      {/* ── Tabs ── */}
      <div style={{ background: "var(--panel)", borderRadius: "12px 12px 0 0", border: "1px solid rgba(96,165,250,0.18)", borderBottom: "none" }}>
      ...
      <div style={{ background: "var(--panel)", border: "1px solid rgba(96,165,250,0.18)", borderTop: "none", borderRadius: "0 0 12px 12px", padding: "1.5rem" }}>
        ...
        {!loading && tab === "dashboard" && (
          <div style={{ display: "grid", gap: "1.25rem" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
              {[
                { label: "Lancamentos do mes", value: totalLanc, suffix: "registros", color: "#93c5fd", bg: "rgba(96,165,250,0.15)" },
                { label: "Pendencias",          value: pendencias, suffix: "rascunhos", color: pendencias > 0 ? "#fbbf24" : "#34d399", bg: pendencias > 0 ? "rgba(251,191,36,0.15)" : "rgba(52,211,153,0.15)" },
                { label: "Conciliacoes abertas",value: concAbertas, suffix: "pendentes", color: concAbertas > 0 ? "#fbbf24" : "#34d399", bg: concAbertas > 0 ? "rgba(251,191,36,0.15)" : "rgba(52,211,153,0.15)" },
                { label: "Resultado",           value: resultado >= 0 ? fmt(resultado) : `(${fmt(resultado)})`, suffix: resultado >= 0 ? "lucro" : "prejuizo", color: resultado >= 0 ? "#34d399" : "#f87171", bg: resultado >= 0 ? "rgba(52,211,153,0.15)" : "rgba(248,113,113,0.15)" },
              ].map((k) => (
              ...
              <div className="list-panel">
                ...
                <div style={{ padding: "0.5rem 0 0.75rem" }}>
                  {pendencias === 0 && concAbertas === 0 && (
                    <div style={{ padding: "1rem", textAlign: "center", color: "#34d399", fontSize: "0.85rem" }}>Nenhuma pendencia!</div>
                  )}
                  {pendencias > 0 && (
                    <button
                      ...
                      type="button"
                    >
                      <span style={{ fontSize: "0.82rem", color: "#fbbf24", fontWeight: 600 }}>{pendencias} lancamento(s) em rascunho</span>
                    </button>
                  )}
                  {concAbertas > 0 && (
                    <button
                      ...
                      type="button"
                    >
                      <span style={{ fontSize: "0.82rem", color: "#fbbf24", fontWeight: 600 }}>{concAbertas} movimentos nao conciliados</span>
                    </button>
                  )}
                </div>
              </div>
                ...
                <div style={{ padding: "0.25rem 0 0.75rem" }}>
                  ...
                  {lancamentos.slice(0, 5).map((entry) => (
                    <div key={entry.id} style={{ padding: "7px 1rem", borderBottom: "1px solid #f0f6ff" }}>
                      <p style={{ margin: "0 0 1px", fontSize: "0.8rem", fontWeight: 700, color: "var(--ink)" }}>{entry.historico}</p>
                  ...
                  {contasVisiveis().map((conta) => {
                    ...
                    return (
                      <tr key={conta.id} style={{ background: conta.nivel === 1 ? "#f8faff" : "transparent" }}>
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 6, paddingLeft: (conta.nivel - 1) * 20 }}>
                            {temFilhos ? (
                              <button
                                ...
                                style={{ background: "none", border: "none", cursor: "pointer", color: "#93c5fd", fontSize: "0.7rem", padding: "0 2px", fontWeight: 800 }}
                                type="button"
                              >
                                {expanded ? "v" : ">"}
                              </button>
                            ) : <span style={{ display: "inline-block", width: 16 }} />}
                            <span style={{ fontWeight: conta.nivel === 1 ? 800 : conta.nivel === 2 ? 600 : 400, fontSize: conta.nivel === 1 ? "0.9rem" : "0.85rem", color: conta.nivel === 1 ? "var(--ink)" : "#374151" }}>
                              {conta.nome}
                            </span>
                          </div>
                        </TD>
                        <TD>
                          <span style={{ fontSize: "0.72rem", fontWeight: 700, color: TIPO_CONTA_COLOR[conta.tipo] ?? "var(--muted)", background: `${TIPO_CONTA_COLOR[conta.tipo] ?? "var(--muted)"}18`, borderRadius: 999, padding: "2px 8px" }}>
                        ...
                        <TD right>
                          <strong style={{ color: conta.tipo === "ativo" || conta.tipo === "despesa" || conta.tipo === "custo" ? "#93c5fd" : "#34d399" }}>
                            {conta.nivel >= 2 ? fmt(conta.saldo) : ""}
                          </strong>
                        </TD>
                        <TD>
                          <Badge {...(conta.ativo ? { bg: "rgba(96,165,250,0.15)", color: "#93c5fd", label: "Ativa" } : { bg: "rgba(148,163,184,0.15)", color: "#9ca3af", label: "Inativa" })} />
                        </TD>
                      </tr>
                    );
                  })}
        ...
        {!loading && tab === "lancamentos" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            ...
            {/* Editor rapido */}
            {novoLanc && (
              <div style={{ background: "#f8faff", border: "1px solid rgba(96,165,250,0.35)", borderRadius: 12, padding: "1.25rem" }}>
                <p style={{ margin: "0 0 1rem", fontWeight: 800, fontSize: "0.875rem", color: "#93c5fd" }}>Novo lancamento</p>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ display: "flex", gap: 20, fontSize: "0.8rem" }}>
                    <span style={{ color: "#93c5fd" }}>D: <strong>{ldebito || "-"}</strong></span>
                    <span style={{ color: "#34d399" }}>C: <strong>{lcredito || "-"}</strong></span>
                    <span style={{ color: lvalor ? "#34d399" : "#9ca3af" }}>Valor: <strong>{lvalor ? fmt(parseFloat(lvalor.replace(",", ".")) || 0) : "-"}</strong></span>
                  </div>
                  <button
                ...
                <tbody>
                  {lancamentos.map((l) => (
                    <tr key={l.id} style={{ background: l.status === "rascunho" ? "#fffbf0" : "transparent" }}>
                      ...
                      <TD>{l.historico}</TD>
                      <TD><span style={{ fontFamily: "monospace", fontSize: "0.78rem", color: "#f87171", fontWeight: 600 }}>{l.conta_debito}</span></TD>
                      <TD><span style={{ fontFamily: "monospace", fontSize: "0.78rem", color: "#34d399", fontWeight: 600 }}>{l.conta_credito}</span></TD>
                      <TD right bold>{fmt(l.valor)}</TD>
                      <TD muted>{l.centro_custo}</TD>
                      <TD><Badge {...(S_LANC[l.status] ?? { bg: "rgba(148,163,184,0.15)", color: "var(--muted)", label: l.status })} /></TD>
                      <TD right>
                        <select
                          className="input"
        ...
        {!loading && tab === "conciliacao" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
              {[
                { label: "Total movimentos", value: String(conciliacao.length), color: "#93c5fd", bg: "rgba(96,165,250,0.15)" },
                { label: "Conciliados", value: String(conciliacao.filter((c) => c.status === "conciliado").length), color: "#34d399", bg: "rgba(52,211,153,0.15)" },
                { label: "Itens pendentes", value: String(concAbertas), color: concAbertas > 0 ? "#fbbf24" : "#34d399", bg: concAbertas > 0 ? "rgba(251,191,36,0.15)" : "rgba(52,211,153,0.15)" },
              ].map((k) => (
                ...
                <tbody>
                  {conciliacao.map((m) => (
                    <tr key={m.id}>
                      ...
                      <TD>{m.descricao}</TD>
                      <TD>
                        <span style={{ fontSize: "0.75rem", fontWeight: 700, color: m.tipo === "entrada" ? "#34d399" : "#f87171" }}>
                          {m.tipo === "entrada" ? "Entrada" : "Saida"}
                        </span>
                      </TD>
                      <TD right>
                        <strong style={{ color: m.tipo === "entrada" ? "#34d399" : "#f87171" }}>
                          {m.tipo === "entrada" ? "+" : "-"} {fmt(m.valor)}
                        </strong>
                      </TD>
                      <TD muted>
                        {m.lanc_id
                          ? <span style={{ fontFamily: "monospace", fontSize: "0.78rem", color: "#93c5fd" }}>Lanc. #{m.lanc_id}</span>
                          : "-"}
                      </TD>
                      <TD><Badge {...(S_CONC[m.status] ?? { bg: "rgba(148,163,184,0.15)", color: "var(--muted)", label: m.status })} /></TD>
                      <TD right>
