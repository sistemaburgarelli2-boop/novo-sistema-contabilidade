...
const SETORES: { key: Setor; label: string; emoji: string; cor: string }[] = [
  { key: "contabilidade", label: "Contabilidade", emoji: "📚", cor: "#93c5fd" },
  { key: "fiscal", label: "Fiscal", emoji: "🏛️", cor: "#34d399" },
  { key: "trabalhista", label: "Trabalhista", emoji: "👥", cor: "#c4b5fd" },
  { key: "societario", label: "Societario", emoji: "🏗️", cor: "#fbbf24" },
  { key: "financeiro", label: "Financeiro", emoji: "💰", cor: "#0891b2" },
  { key: "dp", label: "Dep. Pessoal", emoji: "📋", cor: "#be185d" },
  { key: "legislacao", label: "Legislacao", emoji: "⚖️", cor: "#f87171" },
];
...
export default function EscolaPage() {
  ...
  const inputStyle: React.CSSProperties = { width: "100%", padding: "0.55rem 0.875rem", border: "1.5px solid var(--border)", borderRadius: 8, fontSize: "0.85rem", outline: "none", boxSizing: "border-box" };
  const labelStyle: React.CSSProperties = { display: "block", fontSize: "0.72rem", fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", marginBottom: 4 };

  return (
    <AppShell>
      <div className="page-stack">
        {/* ── Header ── */}
        <div style={{ borderRadius: 14, overflow: "hidden", background: "linear-gradient(110deg, #06170d 0%, #0b2e18 50%, #10b981 100%)", boxShadow: "0 4px 24px rgba(6,23,13,0.18)", padding: "1.5rem 2rem", color: "#ffffff", position: "relative" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", position: "relative", zIndex: 1 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <button onClick={() => { setShowNova(true); setNovoTitulo(""); setNovoConteudo(""); setNovoSetor("contabilidade"); }} style={{ padding: "0.5rem 1rem", background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.25)", borderRadius: 8, color: "#ffffff", fontSize: "0.82rem", fontWeight: 700, cursor: "pointer" }} type="button">
                + Nova anotacao
              </button>
            </div>
          </div>
        ...
        {showNova ? (
          <div>
            <button onClick={() => setShowNova(false)} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: "0.82rem", fontWeight: 600, marginBottom: 12, padding: 0 }} type="button">
            ...
            <div style={{ background: "var(--panel)", border: "1.5px solid var(--border)", borderRadius: 14, padding: "1.5rem", maxWidth: 800 }}>
              <h3 style={{ margin: "0 0 16px", fontSize: "1rem", fontWeight: 700, color: "var(--ink)" }}>Nova anotacao</h3>
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ display: "flex", gap: 12 }}>
                  ...
                  <div style={{ minWidth: 180 }}>
                    ...
                    <select style={{ ...inputStyle, background: "var(--panel)", cursor: "pointer" }} value={novoSetor} onChange={e => setNovoSetor(e.target.value as Setor)}>
                ...
                <button disabled={!novoTitulo.trim()} onClick={criarAnotacao} style={{ padding: "0.65rem 2rem", background: !novoTitulo.trim() ? "#d1d5db" : "linear-gradient(135deg, #4f46e5, #6366f1)", color: "#ffffff", border: "none", borderRadius: 8, fontWeight: 700, fontSize: "0.875rem", cursor: !novoTitulo.trim() ? "not-allowed" : "pointer", alignSelf: "flex-start" }} type="button">
                  Criar anotacao
                </button>
              </div>
            </div>
          ...
          <div>
            <button onClick={() => setLendo(null)} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: "0.82rem", fontWeight: 600, marginBottom: 12, padding: 0 }} type="button">
              <svg fill="none" height={14} viewBox="0 0 24 24" width={14}><path d="M15 18l-6-6 6-6" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5}/></svg>
              Voltar
            </button>
            <div style={{ background: "var(--panel)", border: "1.5px solid var(--border)", borderRadius: 14, padding: "2rem", maxWidth: 800 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                <div>
                  ...
                  <h2 style={{ margin: 0, fontSize: "1.2rem", fontWeight: 800, color: "var(--ink)" }}>{lendo.titulo}</h2>
                  <p style={{ margin: "4px 0 0", fontSize: "0.75rem", color: "#9ca3af" }}>Atualizado em {fmtData(lendo.atualizado_em)}</p>
                </div>
                <div style={{ display: "flex", gap: 6 }}>
                  <button onClick={() => setEditando({ ...lendo })} style={{ padding: "6px 14px", background: "rgba(148,163,184,0.15)", border: "1px solid var(--border)", borderRadius: 8, fontSize: "0.78rem", fontWeight: 600, cursor: "pointer", color: "var(--ink)" }} type="button">Editar</button>
                  <button onClick={() => excluir(lendo.id)} style={{ padding: "6px 14px", background: "rgba(248,113,113,0.15)", border: "1px solid rgba(248,113,113,0.35)", borderRadius: 8, fontSize: "0.78rem", fontWeight: 600, cursor: "pointer", color: "#f87171" }} type="button">Excluir</button>
                </div>
              </div>
          ...
          /* ── Edicao ── */
          <div>
            <button onClick={() => setEditando(null)} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: "0.82rem", fontWeight: 600, marginBottom: 12, padding: 0 }} type="button">
              <svg fill="none" height={14} viewBox="0 0 24 24" width={14}><path d="M15 18l-6-6 6-6" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5}/></svg>
              Cancelar
            </button>
            <div style={{ background: "var(--panel)", border: "1.5px solid var(--border)", borderRadius: 14, padding: "1.5rem", maxWidth: 800 }}>
              <h3 style={{ margin: "0 0 16px", fontSize: "1rem", fontWeight: 700, color: "var(--ink)" }}>Editar anotacao</h3>
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ display: "flex", gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    <label style={labelStyle}>Titulo</label>
                  ...
                  <div style={{ minWidth: 180 }}>
                    <label style={labelStyle}>Setor</label>
                    <select style={{ ...inputStyle, background: "var(--panel)", cursor: "pointer" }} value={editando.setor} onChange={e => setEditando({ ...editando, setor: e.target.value as Setor })}>
                      {SETORES.map(s => <option key={s.key} value={s.key}>{s.emoji} {s.label}</option>)}
                    </select>
                  </div>
                </div>
                ...
                <button onClick={salvarEdicao} style={{ padding: "0.65rem 2rem", background: "linear-gradient(135deg, #4f46e5, #6366f1)", color: "#ffffff", border: "none", borderRadius: 8, fontWeight: 700, fontSize: "0.875rem", cursor: "pointer", alignSelf: "flex-start" }} type="button">
                  Salvar
                </button>
              </div>
            </div>
              ...
              {SETORES.map(s => {
                ...
                return (
                  <button key={s.key} onClick={() => setFiltroSetor(ativo ? "todos" : s.key)} type="button" style={{
                    background: ativo ? `${s.cor}12` : "var(--panel)", border: `1.5px solid ${ativo ? s.cor : "var(--border)"}`,
                    borderRadius: 12, padding: "12px 14px", cursor: "pointer", textAlign: "left", transition: "all 0.15s",
                  }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                      <span style={{ fontSize: 18 }}>{s.emoji}</span>
                      <span style={{ fontWeight: 700, fontSize: "0.82rem", color: ativo ? s.cor : "var(--ink)" }}>{s.label}</span>
                    </div>
                ...
                {filtradas.map(a => {
                  ...
                  return (
                    <button key={a.id} onClick={() => setLendo(a)} type="button" style={{
                      background: "var(--panel)", border: "1.5px solid var(--border)", borderRadius: 12, padding: "1rem 1.25rem",
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 700, fontSize: "0.9rem", color: "var(--ink)", marginBottom: 2 }}>{a.titulo}</div>
      ...
      {/* ── Toast ── */}
      {toast && (
        <div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 9999, display: "flex", alignItems: "center", gap: 10, padding: "14px 20px", borderRadius: 12, background: "#34d399", color: "#ffffff", fontSize: "0.875rem", fontWeight: 600, boxShadow: "0 8px 30px rgba(0,0,0,0.2)", animation: "toastIn 0.3s ease" }}>
